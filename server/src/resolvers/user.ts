...
export class UserResolver {
  ...
  async register(
      ...
      if (errors) {
        ...
        const userId= v4()        
        try {
            await User.create({userId:userId, username: input.username, email: input.email, password: hashedPsw, role: "customer", phone:input.phone,address:input.address }).save()
        } catch (err) {
